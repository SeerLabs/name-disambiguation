...
class PubmedAuthor(Author):
    ...
    def __init__(self, result=None):
        if result is not None:
            ...
            self.affil = result[3].lower().strip()
            if self.affil is None:
                self.affil = ""
            self.order = int(result[4])
            self.affil_bow = dict()
    ...
    def set_name(self, name):
        ...
        if match:
            ...
            if match.group(6) is not None:
                ...
                if self.firstName is not None and len(self.firstName) == 2 and self.firstName[1] >= 'A' and self.firstName[1] <= 'Z':
